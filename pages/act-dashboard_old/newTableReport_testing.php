...
<?php if($parentgroup!=0&&$parentgroup!="")
	...
	while ($reportdata_act = $ObjKfiDash1->dbFetchArray()) {
		...
		$wqtresult =$ObjKfiDash2->dbQuery($wgt_query);
		$wgtdata= $ObjKfiDash2->dbFetchArray();
   ...
   $reportquery ="select min(b.startdate) as startdate , max(b.enddate) as enddate, sum(b.baseline) as baseline from (select a.startdate, a.enddate, a.baseline ,a.itemid From activity a where itemid IN (SELECT itemid FROM maindata WHERE parentgroup LIKE '".$reportdata_act["parentgroup"]."%' AND isentry=1 GROUP BY activitylevel, parentcd ORDER BY maindata.activitylevel)) b";
			...
			$reportdata = $ObjKfiDash3->dbFetchArray();
			 ...
			 $last_date=$ObjKfiDash5->GetlastDateOutput($reportdata_act["parentgroup"]);
			 ...
			 $till_today_qty=$ObjKfiDash6->ActualProgressOutput($reportdata_act["parentgroup"]);
			 $planned_progress=$ObjKfiDash7->PlannedProgressOutput($reportdata_act["parentgroup"],$current_date_c,$reportdata['startdate']);
			...
			{
			$case=2;
			$actual_finish_date=$ObjKfiDash8->ActualFinishDateActivity($next_level_id);
			}
			elseif($current_date_c<$reportdata["enddate"]&&$remaining>0&&$till_today_qty!=0)
			{
			$case=3;
			...
			if($actual_finish_date>$reportdata['startdate'])
			 {
			 $numberDaysElapsed  =$ObjKfiDash9->CalculateElapsedDays($actual_finish_date,$reportdata['startdate']);
...
<td style="text-align:right;"><?php echo number_format($till_today_qty/$reportdata["baseline"]*100,2) ." %";?></td>
</tr>
<?php
